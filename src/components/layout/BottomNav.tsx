...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[480px] bg-white border-t border-gray-100 z-40">
      <div className="flex">
        {navItems.map((item) => {
          const href = `/${hid}${item.key ? `/${item.key}` : ""}`;
          const active = item.key === ""
            ? pathname === `/${hid}`
            : pathname.startsWith(`/${hid}/${item.key}`);
          return (
            <Link
              key={item.key}
              href={href}
      ...
      {/* 홈 인디케이터 영역 — viewport-fit:cover 필요 */}
      <div style={{ height: "calc(env(safe-area-inset-bottom, 0px) + 4px)" }} />
    </nav>
  );
}
